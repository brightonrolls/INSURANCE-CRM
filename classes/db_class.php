...
function select($table,$where=null,$select='*',$order=null,$custom=null) {
$this->connect();

$where=($where!=null?'WHERE '.$where:null);
$order=($order!=null?'ORDER BY '.$order:null);
$custom=($custom!=null?$custom:null);

$q = mysqli_query($this->sql,"SELECT $select FROM $table $where $order $custom");
if ($q)
{
$rows=array();
